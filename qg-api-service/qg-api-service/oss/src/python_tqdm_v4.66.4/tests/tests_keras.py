...
def test_keras(capsys):
    ...
    model.fit(
        ...
        callbacks=[
            TqdmCallback(
                epochs, desc="training", data_size=len(x), batch_size=batch_size, verbose=0
            )
        ],
    )
    ...
    model.fit(
        ...
        callbacks=[
            TqdmCallback(
                epochs, desc="training", data_size=len(x), batch_size=batch_size, verbose=2
            )
        ],
    )
    _, res = capsys.readouterr()
    ...
    model.fit(
        ...
        verbose=False,
        callbacks=[
            TqdmCallback(desc="training", verbose=0, miniters=1, mininterval=0, maxinterval=0)
        ],
    )
    _, res = capsys.readouterr()
    assert "training: " in res
