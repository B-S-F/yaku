...
def test_format_meter():
    ...
    assert (
        format_meter(
            20, 100, 12, ncols=13, rate=8.1, bar_format=r"{l_bar}{bar}|{n_fmt}/{total_fmt}"
        )
        == " 20%|" + unich(0x258F) + "|20/100"
    )
    assert (
        format_meter(
            20, 100, 12, ncols=14, rate=8.1, bar_format=r"{l_bar}{bar}|{n_fmt}/{total_fmt}"
        )
...
def test_smoothing():
    ...
    with closing(StringIO()) as our_file2:
        with closing(StringIO()) as our_file:
            t = tqdm(
                range(3), file=our_file2, smoothing=None, leave=True, miniters=1, mininterval=0
            )
            cpu_timify(t, timer)

            with tqdm(
                range(3), file=our_file, smoothing=None, leave=True, miniters=1, mininterval=0
            ) as t2:
                cpu_timify(t2, timer)
    ...
    with closing(StringIO()) as our_file2:
        with closing(StringIO()) as our_file:
            t = tqdm(
                range(3), file=our_file2, smoothing=1, leave=True, miniters=1, mininterval=0
            )
            cpu_timify(t, timer)

            with tqdm(
                range(3), file=our_file, smoothing=1, leave=True, miniters=1, mininterval=0
            ) as t2:
                cpu_timify(t2, timer)
    ...
    with closing(StringIO()) as our_file2:
        with closing(StringIO()) as our_file:
            t = tqdm(
                range(3), file=our_file2, smoothing=0.5, leave=True, miniters=1, mininterval=0
            )
            cpu_timify(t, timer)

            t2 = tqdm(
                range(3), file=our_file, smoothing=0.5, leave=True, miniters=1, mininterval=0
            )
            cpu_timify(t2, timer)
...
def test_custom_format():
    ...
    with closing(StringIO()) as our_file:
        for _ in TqdmExtraFormat(
            range(10), file=our_file, bar_format="{total_time}: {percentage:.0f}%|{bar}{r_bar}"
        ):
            pass
    ...
    with closing(StringIO()) as our_file:
        ...
        try:
            ...
            raise DeprecationError(
                "Should not allow manual gui=True without"
...
def test_write():
    ...
    with closing(StringIO()) as our_file:
        ...
        t2 = trange(
            10, file=our_file, desc="pos1 bar", bar_format="{l_bar}", mininterval=0, miniters=1
        )
        t3 = tqdm(
            total=10,
            file=our_file,
...
def test_postfix():
    ...
    with closing(StringIO()) as our_file:
        with tqdm(
            total=10, file=our_file, desc="pos0 bar", bar_format="{r_bar}", postfix=postfix
        ) as t1:
            t1.refresh()
            out = our_file.getvalue()
