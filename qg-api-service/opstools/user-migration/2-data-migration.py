...
def check_and_create_user_if_missing(
    username: str, usermap: dict, fakeusers: dict, logger: Logger
) -> dict:
    ...
    for user in usermap.values():
        if user["id"] == id:
            raise RuntimeError(
                "Randomly generated UUID already assigned. Implementation error."
            )
...
def migrate_audit_table(
    conn: Connection, table_name: str, usermap: dict, fakeusers: dict, logger: Logger
):
    with conn.cursor() as cur:
        ...
        for record in records:
            ...
            if len(diff) != 0:
                ...
                cur.execute(
                    sql.SQL("UPDATE {} SET actor = %s WHERE id = %s").format(
                        sql.Identifier(table_name)
                    ),
                    [newJson, rowid],
                )
            ...
            if original and "createdBy" in original and "lastModifiedBy" in original:
                ...
                if UUID_PATTERN.match(createdBy) is None:
                    user = check_and_create_user_if_missing(
                        createdBy, usermap, fakeusers, logger
                    )
                    ...
                    cur.execute(
                        sql.SQL(
                            """UPDATE {} SET original = jsonb_set(original, {}, {}) WHERE id = %s"""
                        ).format(
                            sql.Identifier(table_name),
                            sql.Literal("{createdBy}"),
                            sql.Literal(f'''"{user['id']}"'''),
                        ),
                        [rowid],
                    )
                ...
                if UUID_PATTERN.match(lastModifiedBy) is None:
                    user = check_and_create_user_if_missing(
                        lastModifiedBy, usermap, fakeusers, logger
                    )
                    logger.debug(f"{table_name} {rowid}")
            ...
            if modified and "createdBy" in modified and "lastModifiedBy" in modified:
                ...
                if UUID_PATTERN.match(createdBy) is None:
                    ...
                    cur.execute(
                        sql.SQL(
                            """UPDATE {} SET modified = jsonb_set(modified, {}, {}) WHERE id = %s"""
                        ).format(
                            sql.Identifier(table_name),
                            sql.Literal("{createdBy}"),
                            sql.Literal(f'''"{user['id']}"'''),
    ...
    with conn.cursor() as cur:
        ...
        for record in records:
            ...
            if UUID_PATTERN.match(createdBy) is None:
                ...
                cur.execute(
                    sql.SQL("""UPDATE {} SET "createdBy" = %s WHERE id = %s""").format(
                        sql.Identifier(table_name)
                    ),
                    [user["id"], rowid],
                )
            ...
            if UUID_PATTERN.match(lastModifiedBy) is None:
                ...
                cur.execute(
                    sql.SQL("""UPDATE {} SET "lastModifiedBy" = %s WHERE id = %s""").format(
                        sql.Identifier(table_name)
                    ),
                    [user["id"], rowid],
                )
...
def extra_migrate_approval_table(
    conn: Connection, usermap: dict, fakeusers: dict, logger: Logger
):
    with conn.cursor() as cur:
...
def extra_migrate_approval_audit_table(
    conn: Connection, usermap: dict, fakeusers: dict, logger: Logger
):
    with conn.cursor() as cur:
        ...
        for record in records:
            ...
            if original:
                ...
                if UUID_PATTERN.match(approver) is None:
                    user = check_and_create_user_if_missing(
                        approver, usermap, fakeusers, logger
                    )
                    ...
                    cur.execute(
                        sql.SQL(
                            """UPDATE approval_audit SET original = jsonb_set(original, {}, {}) WHERE id = %s"""
                        ).format(
                            sql.Literal("{approver}"),
                            sql.Literal(f'''"{user['id']}"'''),
                        ),
                        [rowid],
                    )
            ...
            if modified:
                ...
                if UUID_PATTERN.match(approver) is None:
                    ...
                    cur.execute(
                        sql.SQL(
                            """UPDATE approval_audit SET modified = jsonb_set(modified, {}, {}) WHERE id = %s"""
                        ).format(
                            sql.Literal("{approver}"),
                            sql.Literal(f'''"{user['id']}"'''),
                        ),
...
def extra_migrate_findings_table(
    conn: Connection, usermap: dict, fakeusers: dict, logger: Logger
):
    with conn.cursor() as cur:
...
def extra_migrate_comments_table(conn: Connection, usermap: dict, logger: Logger):
    with conn.cursor() as cur:
        ...
        for record in records:
            ...
            new_content = MENTION_PATTERN.sub(
                lambda match: replacment_template(match, usermap, logger), content
            )
        ...
        for record in records:
            ...
            if original:
                ...
                if new_content != content:
                    ...
                    cur.execute(
                        sql.SQL(
                            """UPDATE comment_audit SET original = jsonb_set(original, {}, {}) WHERE id = %s"""
                        ).format(
                            sql.Literal("{content}"),
                            sql.Literal(f"""{json.dumps(new_content)}"""),
                        ),
                        [rowid],
                    )
            ...
            if modified:
                ...
                if new_content != content:
                    ...
                    cur.execute(
                        sql.SQL(
                            """UPDATE comment_audit SET modified = jsonb_set(modified, {}, {}) WHERE id = %s"""
                        ).format(
                            sql.Literal("{content}"),
                            sql.Literal(f"""{json.dumps(new_content)}"""),
                        ),
