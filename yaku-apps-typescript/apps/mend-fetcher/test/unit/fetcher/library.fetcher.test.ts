...
import { getLibraryDTOs } from '../../../src/fetcher/library.fetcher'
...
import { librariesData } from '../fixtures/data'
...
describe('library.fetcher', () => {
  ...
  it('should retrieve libraries when there is a single page response', async () => {
  ...
  it('should retrieve libraries when there is a multipage response', async () => {
    const fakeAuth: FakeAuthenticator = new FakeAuthenticator(env)
    const spy = vi.spyOn(axiosInstance, 'request')
    spy.mockReturnValueOnce(
      Promise.resolve({
