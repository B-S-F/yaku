...
import {
  getGitBranchesErrorMockServerResponse,
  getGitBranchesMockServerResponse,
} from './fixtures/getGitBranchesMockServerResponse'

describe('Fetch Branches from Bitbucket', () => {
  const MOCK_SERVER_PORT = 8080
  const getBranchesEndpoint =
  ...
  it('should successfully fetch all three pages of branches', async () => {
    mockServerOptions = getGitBranchesMockServerResponse(MOCK_SERVER_PORT)
    mockServer = new MockServer(mockServerOptions)

    const result: RunProcessResult = await run(gitFetcherExecutable, [], {
      env: {
        ...environment,
        GIT_FETCHER_CONFIG_FILE_PATH: `${__dirname}/configs/git-fetcher-config-fetch-branches-from-bitbucket.yml`,
      },
    })
    ...
    expect(result.stdout[0]).toEqual('Fetched 5 branches')
    expect(result.stdout[1]).toEqual(
      'Fetch from https://localhost:8080 was successful with config {"org":"aquatest","repo":"bitbucket-fetcher-test-repo","resource":"branches"}',
    )
    ...
    expectedPageStart = `${mockServerOptions.responses[getBranchesEndpoint].get[0].responseBody.nextPageStart}`
    ...
    expectedPageStart = `${mockServerOptions.responses[getBranchesEndpoint].get[1].responseBody.nextPageStart}`
    ...
    const expectedFileContent: BitbucketBranch[] = [
      ...mockServerOptions.responses[getBranchesEndpoint].get[0].responseBody
        .values,
      ...mockServerOptions.responses[getBranchesEndpoint].get[1].responseBody
        .values,
      ...mockServerOptions.responses[getBranchesEndpoint].get[2].responseBody
        .values,
    ]
    expect(actualFileContent).toStrictEqual(JSON.stringify(expectedFileContent))
  })

  it('should return failed, if bitbucket returns 404 NOT FOUND error', async () => {
    mockServerOptions = getGitBranchesErrorMockServerResponse(MOCK_SERVER_PORT)
    mockServer = new MockServer(mockServerOptions)
