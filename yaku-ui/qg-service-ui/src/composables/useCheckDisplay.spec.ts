...
describe('useCheckDisplay', () => {
  it('is reactive', () => {
    ...
    statusRef.value = 'GREEN'
    expect(statusDisplay.icon).toStrictEqual(CHECK_DISPLAY_CONFIG.GREEN.icon)
  })
  ...
  it('is extracted prop reactive with toRefs', () => {
    ...
    statusRef.value = 'GREEN'
    expect(icon.value).toStrictEqual(CHECK_DISPLAY_CONFIG.GREEN.icon)
  })
})
