...
<template>
  <VuetifyDashboardTile heading="Decreased Quality" :isEmpty="findings.length === 0">
    ...
    <template #empty>
      <VuetifyEmptyTileContent type="success" label="There are currently no configurations with decreased quality" />
    </template>
...
const populate = async () => {
  ...
  while (findings.value.length < 3 && hasNext) {
    ...
    await apiMetrics
      .getLatestRunFindings({ items: CHUNK, sortBy: 'diff', page })
      .then(async (r) => {
